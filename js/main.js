...
function generateNumbersWithSymbols() {
  ...
  for (let i = 1; i < 100; i++) {
    ...
    // Store the symbol for multiples of 9
    if (i % 9 === 0) {
      symbolMapping[i] = symbol;
    }
  }

  // Store the symbol mapping for multiples of 9 in state
  state.symbolMapping = symbolMapping;

  return numbersWithSymbols;
}

// Update the last page to display the symbol associated with the selected number
